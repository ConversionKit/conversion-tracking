...
- **Destination.** Where the conversion fires to, and how. Each destination reports a delivery mode: **`dual`** (server-side plus an optional browser pixel, the most reliable), **`server`**, or **`browser`** (pixel only, which is how the privacy analytics tools work). **Do not hardcode which platform is which.** Run `converly destinations types` and read the mode off the live catalogue, because it changes as platforms are added and upgraded.
